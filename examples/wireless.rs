...
fn main() {
    ...
    let rollback = args.iter().position(|a| a == "--rollback").map(|i| {
        ...
        match spec.split_once(':') {
            Some((a, l)) => (a.parse().expect("bad frame"), l.parse().expect("bad len")),
            None => (spec.parse().expect("bad frame"), 120usize),
        }
    });
    let rollback_at = rollback.map(|(at, _)| at);
    let rollback_len = rollback.map(|(_, len)| len).unwrap_or(120);
